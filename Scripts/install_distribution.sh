...
APP_SOURCE="$INSTALLER_DIR/BabyRecorder.app"
...
if [[ ! -d "$APP_SOURCE" ]]; then
  echo "BabyRecorder.app was not found next to this installer: $APP_SOURCE" >&2
  exit 1
fi

echo "Installing BabyRecorder app to: $APP_DEST"
...
echo "Preparing transcription runtime at: $RUNTIME_ROOT"
...
if [[ ! -f "$APP_DEST/Contents/Resources/Scripts/transcribe_mlx_qwen3_asr.sh" ]]; then
  echo "Transcription bridge script is missing from BabyRecorder.app." >&2
  exit 1
fi
...
echo "  $RUNTIME_ROOT"
echo
echo "Open BabyRecorder from Applications, grant Microphone and Screen Recording permissions, then start recording."
