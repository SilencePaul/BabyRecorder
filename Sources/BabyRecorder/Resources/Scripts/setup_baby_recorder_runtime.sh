...
install_python_pkg_with_authorization() {
  local pkg_path="$1"
  /usr/bin/osascript - "$pkg_path" <<'OSA'
on run argv
...
install_python_with_pkg() {
  ...
  for url in $PYTHON_PKG_URLS; do
    echo "Downloading Python ${PYTHON_VERSION} from: $url"
    if /usr/bin/curl -fL --connect-timeout 20 --retry 2 --retry-delay 2 -o "$pkg_path" "$url"; then
      echo "Installing Python ${PYTHON_VERSION}. macOS may ask for your password."
      install_python_pkg_with_authorization "$pkg_path"
      rm -rf "$tmp_dir"
      return
    fi
...
if [[ ! -f "$SCRIPT_DIR/transcribe_mlx_qwen3_asr.sh" ]]; then
  echo "Transcription bridge script is missing from app resources." >&2
  exit 1
fi

echo "Preparing BabyRecorder transcription runtime at: $RUNTIME_ROOT"
mkdir -p "$RUNTIME_ROOT/Scripts" "$RUNTIME_ROOT/huggingface"
cp "$SCRIPT_DIR/transcribe_mlx_qwen3_asr.sh" "$SCRIPT_DEST"
chmod +x "$SCRIPT_DEST"
...
PY
  ...
  echo "Skipping model download because BABY_RECORDER_WARM_MODEL=$WARM_MODEL"
fi

echo "BabyRecorder transcription runtime is ready."
